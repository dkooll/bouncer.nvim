...
-- Cache for module versions to avoid redundant requests
local version_cache = {}
...
-- Function to fetch latest major version, with caching and coroutine support
local function get_latest_major_version(registry_source)
  if version_cache[registry_source] then
    return version_cache[registry_source]
  end
      ...
      if result and result.status == 200 and result.body then
        ...
        if data and data.modules and data.modules[1] and data.modules[1].versions then
          ...
          if latest_major_version then
            local version_constraint = "~> " .. latest_major_version .. ".0"
            version_cache[registry_source] = version_constraint -- Cache the result
            coroutine.resume(co, version_constraint)
            return
          end
        end
      end
