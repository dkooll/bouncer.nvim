...
--
-- ---------------------------------------------------------------------------------------------------
-- -- Key fix: Remove strict indentation check so that `source =` lines get replaced
-- --          no matter how they are aligned within the module block.
-- ---------------------------------------------------------------------------------------------------
...
--     -- Not yet in a module block?
--     if not in_module_block then
--       table.insert(new_lines, line)
--       local module_match = line:match(patterns.module_block)
--       if module_match and lines[i + 1] then
--         -- We found the start of a module block, check if next line's source
--         -- matches either our registry source or a local ../../
...
--     end
--
--     -- We are in a module block now. Check for end of block:
...
--
--     -- ----------------------------------------------------------------------
--     -- Remove the strict indentation check here so we catch `source` lines
--     -- no matter how they're aligned within the block.
--     -- ----------------------------------------------------------------------
--
--     -- If the line looks like a `source = ` line, we skip it (don't re-insert)
--     -- and insert our new line(s) if not already done:
--     if line:match('%s*source%s*=') then
--       if not source_added then
--         -- Insert the correct source line
--         if is_local then
...
--     end
--
--     -- If the line looks like a `version = ` line, skip it (don't re-insert):
...
--         end
--         -- Force an edit reload to pick up changes
--         vim.cmd('edit')
--       end
--     end)
--   end
